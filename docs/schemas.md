...
- `repomori.release_rehearsal.v1`: local sanitized release-candidate evidence rehearsal report with reviewer-surface privacy and storage-path checks.
...
- `repomori.release_review_privacy_guard.v1`: release-review decision log privacy guard for local paths, temp directories, secret-like values, private URLs, and raw dump keys, with redacted issue categories/counts.
- `repomori.release_review_privacy_guard_demo.v1`: synthetic clean/failing privacy-guard dry-run report for exercising redacted categories/counts before release review.
